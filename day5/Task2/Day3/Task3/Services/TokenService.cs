...
public class TokenService : ITokenService
{
    ...
    // In-memory store: token value → RefreshToken.
    // Static so the singleton survives across requests; production would use a DB.
    private static readonly Dictionary<string, RefreshToken> _store = new();
    ...
    public (string accessToken, RefreshToken refreshToken)? RotateRefreshToken(string incomingToken)
    {
        ...
        lock (_lock)
        {
            ...
            if (rt.IsRevoked || rt.ExpiresAt < DateTime.UtcNow)
                return null;
