...
### 1. Registered Resource Providers
Three providers were not registered on the subscription; registered with `--wait`:
```
...
```
A logical container for all related Azure resources.

### 3. Container Apps Environment
```
az containerapp env create -n thinkschool-env -g thinkschool-rg -l centralindia
```
Auto-created a Log Analytics workspace (`workspace-thinkschoolrgeUzN`) for built-in observability.
- **defaultDomain**: `purpleflower-cae11894.centralindia.azurecontainerapps.io`
- **staticIp**: `4.224.10.31`
- **kedaVersion**: `2.18.1` (powers HTTP autoscale)
- **daprVersion**: `1.16.4` (sidecar available for service mesh)

### 4. Azure Container Registry
```
az acr create -n thinkschoolacr -g thinkschool-rg --sku Basic --admin-enabled true
```
- **loginServer**: `thinkschoolacr.azurecr.io`

### 5. Dockerfile (Task3/Dockerfile)
Multi-stage Dockerfile for the ASP.NET .NET 10 Quotes API:
- **Build stage**: `mcr.microsoft.com/dotnet/sdk:10.0` — restores NuGet packages and publishes release build
- **Runtime stage**: `mcr.microsoft.com/dotnet/aspnet:10.0` — minimal runtime image, listens on port 8080
- `.dockerignore` excludes all `**/bin/`, `**/obj/`, test projects, and dev-only files
...
| Flag | Purpose |
|------|---------|
| `--ingress external` | Public HTTPS endpoint with free TLS cert |
| `--target-port 8080` | Port the container listens on inside the pod |
| `--min-replicas 0` | Scale to zero when idle (saves cost) |
| `--max-replicas 5` | Upper bound to prevent runaway scaling |
| `--scale-rule-type http` | KEDA-powered HTTP trigger |
| `--scale-rule-http-concurrency 10` | Add a replica for every 10 concurrent requests |

## Revision Model
Every `az containerapp update` (new image, env vars, etc.) creates an immutable **revision**.  
Multiple revisions can coexist for blue-green or canary traffic splitting via:
```
az containerapp ingress traffic set --name quotesapi -g thinkschool-rg \
  --revision-weight revision1=90 revision2=10
```
