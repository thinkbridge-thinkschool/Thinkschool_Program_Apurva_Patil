...
// Each test class instance owns its factory → each test gets its own fresh SQL Server database.
// The shared SqlServerContainerFixture (started once per collection) provides the container;
// IntegrationTestFactory stamps a unique catalog name per instance for full isolation.
