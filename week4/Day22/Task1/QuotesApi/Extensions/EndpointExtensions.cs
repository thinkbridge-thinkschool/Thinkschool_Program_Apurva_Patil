...
public static class EndpointExtensions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Fix 3: hard cap prevents cache-pollution from arbitrarily large page numbers
        if (page > 10_000)
        ...
        // Fix 1: tag every list entry so CreateQuote/DeleteQuote can wipe all pages at once
        // Fix 4: structured log instead of Console.WriteLine (non-blocking, correlates with trace)
        var quotes = await cache.GetOrCreateAsync(
            cacheKey,
            async ct =>
            {
                log.LogInformation("[DB HIT] Fetching quotes from database — page={Page}, size={Size}", page, size);
    ...
    /// GET /api/quotes/{id} - Get quote by ID
    /// </summary>
    // Fix 2: cache individual quotes; DeleteQuote invalidates the key so stale data never serves
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Fix 1: wipe all paginated list entries so new quote is visible immediately
        await cache.RemoveByTagAsync("quotes", cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (deleted)
        {
            // Fix 1: wipe paginated list cache so deleted quote disappears immediately
            await cache.RemoveByTagAsync("quotes", cancellationToken);
            // Fix 2: wipe the individual quote entry for this ID
            await cache.RemoveAsync($"quotes:id={id}", cancellationToken);
        }
