...
public static class EndpointExtensions
{
    public static WebApplication MapQuoteEndpoints(this WebApplication app)
    {
        ...
        // Dapper-backed list — same shape as GET /, used by k6 load test
        group.MapGet("/dapper", GetAllQuotesDapper)
            .WithName("GetAllQuotesDapper")
            .WithDescription("Get all quotes via Dapper (read model)");

        // Declared before /{id} so "benchmark" isn't swallowed by the int-binding route
        group.MapGet("/benchmark", BenchmarkQueries)
            .WithName("BenchmarkQueries")
            .WithDescription("Compare EF Core vs Dapper timing for GetPagedAsync (10 timed runs after 1 warmup)");
    ...
    private static async Task<IResult> GetAllQuotesDapper(
        [FromKeyedServices("dapper")] IQuoteQueryService queryService,
        int page = 1,
        int size = 10,
        CancellationToken cancellationToken = default)
    {
    ...
    // ── BENCHMARK: runs both implementations 10× after a warmup, returns timing comparison ──
    private static async Task<IResult> BenchmarkQueries(
        [FromKeyedServices("ef")] IQuoteQueryService efService,
        [FromKeyedServices("dapper")] IQuoteQueryService dapperService,
        int page = 1,
        int size = 10,
        CancellationToken cancellationToken = default)
    {
        ...
        if (page < 1) page = 1;
        if (size is < 1 or > 100) size = 10;

        // Warmup — primes the EF query plan cache and Dapper's IL mapper
        for (var i = 0; i < warmupRuns; i++)
        {
            await efService.GetPagedAsync(page, size, cancellationToken);
            await dapperService.GetPagedAsync(page, size, cancellationToken);
        }

        var efTimes = new double[timedRuns];
        for (var i = 0; i < timedRuns; i++)
        {
            var sw = Stopwatch.StartNew();
            await efService.GetPagedAsync(page, size, cancellationToken);
            efTimes[i] = sw.Elapsed.TotalMilliseconds;
        }

        var dapperTimes = new double[timedRuns];
        for (var i = 0; i < timedRuns; i++)
        {
            var sw = Stopwatch.StartNew();
            await dapperService.GetPagedAsync(page, size, cancellationToken);
            dapperTimes[i] = sw.Elapsed.TotalMilliseconds;
        }

        var efAvg     = efTimes.Average();
        var dapperAvg = dapperTimes.Average();
        ...
        return Results.Ok(new
        {
            ...
            ef = new
            {
                avgMs = Math.Round(efAvg, 2),
                minMs = Math.Round(efTimes.Min(), 2),
                maxMs = Math.Round(efTimes.Max(), 2),
            ...
            dapper = new
            {
                avgMs = Math.Round(dapperAvg, 2),
                minMs = Math.Round(dapperTimes.Min(), 2),
                maxMs = Math.Round(dapperTimes.Max(), 2),
            ...
            winner   = dapperAvg <= efAvg ? "dapper" : "ef",
            speedupX = Math.Round(efAvg / dapperAvg, 1)
        });
    }
    ...
    public static WebApplication MapCollectionEndpoints(this WebApplication app)
    {
        ...
        app.MapPost("/api/dev/seed", async (QuotesDbContext db, CancellationToken ct) =>
        {
            ...
            // 500 rows so the benchmark numbers are meaningful
